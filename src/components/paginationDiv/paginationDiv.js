...
const PaginationDiv = ({all_items, onItemsChanged, num_in_page, page_num}) => {
    ...
    return (
        <div>
            {all_items.length === 0 ? null :
                <ul className={styles.page_list}>
                    {(currPage !== 1) && <li>
                        <button className={styles.page_btn} onClick={() => goPage(currPage - 1)}>قبلی</button>
                    </li>}
                    {range(1, endPage).map((val, index) => {
                        return (
                            <li key={index}>
                                <button className={`${styles.page_btn} ${currPage === val && styles.selected}`}
                                        onClick={() => goPage(val)}>{val}</button>
                            </li>
                        )
                    })}
                    {(currPage !== endPage) && <li>
                        <button className={styles.page_btn} onClick={() => goPage(currPage + 1)}>بعدی</button>
                    </li>}
                </ul>
            }
        </div>
